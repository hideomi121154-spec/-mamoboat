/* MAMO BOAT Plan System v2.3 — static display only. No click handlers, no observers. */
(() => {
  "use strict";
  if (window.__MAMO_PLAN_SYSTEM_V23__) return;
  window.__MAMO_PLAN_SYSTEM_V23__ = true;
  ...
  function card(plan){
    return `<article class="mamo-plan-card mamo-${plan.id}">
      ${plan.recommended?'<span class="mamo-plan-ribbon">おすすめ</span>':""}
      <div class="mamo-plan-head"><div><small>${plan.name}</small><h3>${plan.price?`${plan.price.toLocaleString("ja-JP")}円`:'無料'}<em>${plan.price?'/月':''}</em></h3></div></div>
  ...
  function render(){
    ...
    panel.innerHTML=`
      ...
      <div class="mamo-plan-grid">${PLANS.map(card).join("")}</div>
