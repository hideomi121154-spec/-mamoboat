...
const CACHE = "mamoboat-v401-central-pilot-5";
...
self.addEventListener("notificationclick",event=>{
  ...
  event.waitUntil((async()=>{
    ...
    const appClient=list.find(client=>String(client.url||"").startsWith(scope));
    if(appClient){
      try{appClient.postMessage({type:"MAMO_OPEN_MORNING_PRESS",url:target})}catch(_){}
      try{if("navigate" in appClient)await appClient.navigate(target)}catch(_){}
      try{await appClient.focus()}catch(_){}
      return;
    }
