...
test("the decision-conflict modules are available offline", () => {
  ...
  assert.match(serviceWorker, /mamoboat-v417-air-bet-required-stake-55-dev/);
});

test("AIR BET amount edits remain observable after the compact cart update", () => {
  assert.match(pilot, /decision-conflict-guard\.js\?v=20260906-2/);
  assert.match(guard, /#allStakeInput, #reviewAllStakeInput, \.cart-stake-input, \.betline-stake-input/);
  assert.match(guard, /#cartTools button, #reviewStakeTools button/);
