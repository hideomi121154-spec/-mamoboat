...
(() => {
  ...
  function ensureSheet() {
    ...
    bg.className = "mr-bg";
    bg.innerHTML = `<div class="mr-sheet" role="dialog" aria-modal="true" aria-label="MAMO RECORD"><div id="mamoRecordSheetBody"></div></div>`;
  ...
  function telemetry(eventName, payload, record) {
    ...
    const context = { screen: document.body?.dataset?.screen || "race", raceDate: record?.raceDate || null, venueCode: record?.venueCode || null, raceNo: record?.raceNo || null };
  ...
  function completeReflection() {
    ...
    const s = recordState();
    s.reflections[id] = { recordId: id, type: "air_bet_after", conviction: Number(answers.conviction), cashUrge: Number(answers.cashUrge), recordedAt: new Date().toISOString(), raceDate: activeRecord.raceDate || null, venueCode: activeRecord.venueCode || null, venue: activeRecord.venue || null, raceNo: activeRecord.raceNo || null, stakeB: Number(activeRecord.stake) || 0 };
    const award = awardReflection(s, id);
    saveRecordState(s);
    telemetry("mamo_record_reflection_completed", { record_id: id, conviction: Number(answers.conviction), cash_urge: Number(answers.cashUrge), record_awarded: award.amount, daily_record_cap: DAILY_CAP, reward_basis: "reflection_completion_only" }, activeRecord);
    renderSummary();
  ...
  function renderSummary() {
    ...
    if (!panel) { panel = document.createElement("div"); panel.id = "mamoRecordSummary"; stats.insertAdjacentElement("afterend", panel); }
    const s = recordState();
  ...
  function scanNewAirBets() {
    ...
    if (!scanReady) {
      records.forEach((r) => { const id = stableId(r); if (id) { scanSeen.add(id); persistedSeen.add(id); } });
      rs.seenRecordIds = [...persistedSeen].slice(-3000); saveRecordState(rs); scanReady = true; return;
    }
    for (const record of records) {
      const id = stableId(record);
      if (!id || scanSeen.has(id) || persistedSeen.has(id)) continue;
      scanSeen.add(id); persistedSeen.add(id); rs.seenRecordIds = [...persistedSeen].slice(-3000); saveRecordState(rs);
      if (!rs.reflections[id]) setTimeout(() => { if (!activeRecord) showFor(record); }, 300);
      break;
    }
  }

  function boot() {
    installStyle(); ensureSheet();
    document.getElementById("mamoRecordSheetBg")?.addEventListener("click", handleSheetClick, false);
    renderSummary(); scanNewAirBets();
    ...
    window.MAMO_RECORD = Object.freeze({ version: 1, balance: () => recordState().balance, state: () => recordState(), dailyCap: DAILY_CAP });
  }
