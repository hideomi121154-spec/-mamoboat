...
(() => {
  ...
  function boot(){style();ensureSnapshotHydration();document.addEventListener("click",e=>{if(e.target?.closest?.(".mamo-carte-btn,[data-race-carte-index],[data-rx-carte]")){ensureSnapshotHydration();schedule()}},true);window.addEventListener("mamo:race-carte-snapshot",schedule);schedule();}
